# implements TANE algorithm
# ref: https://dm-gatech.github.io/CS8803-Fall2018-DML-Papers/tane.pdf
...
def findCplus(x):
    ...
    thesets=[]
    for a in x:
        if x.replace(a,'') in list(dictCplus.keys()):
...
def compute_dependencies(level, listofcols):
    ...
    for x in level:
        ...
        if list(set.intersection(*thesets)) == []:
            ...
            dictCplus[x] = list(set.intersection(*thesets))
    for x in level:
        for a in x:
            if a in dictCplus[x]:
    ...
    for x in level: # line 1
        ...
        if check_superkey(x): # line 4   ### should this check for a key, instead of super key??? Not sure.
            ...
            for a in temp: # line 5
                thesets=[]
                for b in x:
                    if not( ''.join(sorted((x+a).replace(b,''))) in list(dictCplus.keys())): 
...
def find_fd_from_csv(infile):
    ...
    FD_dict = {}
    for f in finallistofFDs:
        if f[0] in FD_dict:
            FD_dict[f[0]].append(f[1])
        else:
            FD_dict[f[0]] = [f[1]]
